...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void cargarDentistas() {
	    ...
	    try {
	        List<Dentista> dentistas = em.createQuery("SELECT d FROM Dentista d", Dentista.class).getResultList();
	...
	void listar() {
	    ...
	    try {
	    	List<EquipoDental> equipos = em.createQuery("SELECT e FROM EquipoDental e ORDER BY e.nroEquipo ASC", EquipoDental.class).getResultList();

	        txtSalida.setText(""); 
	        
	        for (EquipoDental e : equipos) {
	            imprimir("Nro Equipo: " + e.getNroEquipo());
	            imprimir("Nombre: " + e.getNombre());
	            imprimir("Costo: " + e.getCosto());
	            imprimir("Fecha Adquisición: " + e.getFechaAdquisicion());
	            imprimir("Dentista: " + (e.getDentista() != null ? e.getDentista().getNombreCompleto() : "Sin asignar"));
	            imprimir("Correo: " + (e.getDentista() != null ? e.getDentista().getCorreo() : "Sin correo"));
	            imprimir("Especialidad: " + (e.getDentista() != null ? e.getDentista().getEspecialidad().getTitulo() : "Sin especialidad"));
	...
	void adicionar() {
	    ...
	    try {
	        ...
	        char estado = cboEstados.getSelectedItem().toString().charAt(0);
	        Dentista dentista = (Dentista) cboDentistas.getSelectedItem();

	        EquipoDental equipo = new EquipoDental(nombre, costo, estado, dentista);
	        em.persist(equipo);
	...
	void buscar() {
	    ...
	    try {
	        EquipoDental equipo = em.find(EquipoDental.class, Integer.parseInt(txtNroEquipo.getText()));
	        ...
	        cboEstados.setSelectedItem(String.valueOf(equipo.getEstado()));
	        cboDentistas.setSelectedItem(equipo.getDentista());
	        txtFechaAdquisicion.setText(equipo.getFechaAdquisicion().toString());
	...
	void modificar() {
	    ...
	    try {
	        ...
	        equipo.setEstado(cboEstados.getSelectedItem().toString().charAt(0));
	        equipo.setDentista((Dentista) cboDentistas.getSelectedItem());
